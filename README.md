...
* [ ] 对TP自带的Auth类做了全面的抽象和重写，功能更细致，更完善，与系统彻底解耦（可以把类提出来，放到YII或者laravel里用都没问题），参考` app\common\tool\permission\Auth;
`
...
* [ ]  可以对页面里的单个元素（一个按钮，表格里的一个字段等）做精细化控制，是否可见，是否可直接编辑。比如设置某表格，在A角色登陆时，当B字段的值为5的时候，C字段是否可编辑，删除按钮是否可用等，十分灵活，只需要编写配置文件即可 ，参考` app\common\tool\view\Auth;` 配置文件参考` app\extra
`
...
## 架构
* [ ] 基于TP5编写就意味着TP5官网声明的功能都可以随意搭配，包括分布式、负载均衡、集群、Redis、分库分表、nosql缓存等各种机制，参考TP5官方文档 https://www.kancloud.cn/manual/thinkphp5/122950
