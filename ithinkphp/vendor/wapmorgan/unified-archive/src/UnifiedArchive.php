<?php
	...
	class UnifiedArchive extends BasicArchive
	{
		...
		public static function open($fileName)
		{
			...
			if(!file_exists($fileName) || !is_readable($fileName)) throw new Exception('Count not open file: ' . $fileName);
		...
		public static function detectArchiveType($fileName , $contentCheck = true)
		{
			...
			if(in_array($ext , [
					...
					'tbz2' ,
					'txz',
				]) || preg_match('~\.tar\.(gz|bz2|xz|z)$~' , strtolower($fileName)))
			{
			...
			if($contentCheck)
			{
				...
				if($type = TarArchive::detectArchiveType($fileName)) return $type;
			}
		...
		public function pclzipInterace()
		{
			...
			throw new Exception(basename(__FILE__) . ', line ' . __LINE__ . ' : PclZip-like interface IS' . 'NOT available for ' . $this->type . ' archive format');
		}
		...
		{
			if(!in_array($fileName , $this->files , true)) return false;
			...
			switch ($this->type)
			{
				case self::ZIP:
					...
					return new ArchiveEntry($fileName , $stat['comp_size'] , $stat['size'] , $stat['mtime'] , $stat['comp_method'] != 0);

				case self::SEVEN_ZIP:
					$entry = $this->seven_zip->getEntry($fileName);
					$size = $entry->getSize();

					return new ArchiveEntry($fileName , $size , ceil($size * ($this->compressedFilesSize / $this->uncompressedFilesSize)) , strtotime($entry->getModified()) , $this->compressedFilesSize != $this->uncompressedFilesSize);

				case self::RAR:
					$entry = $this->rar->getEntry($fileName);

					return new ArchiveEntry($fileName , $entry->getPackedSize() , $entry->getUnpackedSize() , strtotime($entry->getFileTime()) , $entry->getMethod() != 48);

				case self::GZIP:
				case self::BZIP:
				case self::LZMA:

					return new ArchiveEntry($fileName , $this->archiveSize , $this->type === self::GZIP ? $this->gzipStat['size'] : $this->archiveSize , $this->type === self::GZIP ? $this->gzipStat['mtime'] : ($this->type === self::BZIP ? $this->bzipStat['mtime'] : 0) , true);
		...
		{
			if($expandFilesList && $files !== null) $files = self::expandFileList($this->files , is_string($files) ? [$files] : $files);
			...
			switch ($this->type)
			{
				...
				case self::SEVEN_ZIP:
					...
					else
					{
						foreach ($this->files as $fname)
						{
							if(strpos($fname , $files) === 0)
							{
								if($this->seven_zip->extractEntry($fname)) $count++;
							}
						}
					}
				...
				case self::GZIP:
					if($files === null || $files === $this->files)
					{
						...
						if(file_put_contents($dir . basename($this->gzipFilename , '.gz') , gzdecode(file_get_contents($this->gzipFilename))) !== false) return 1;
						else
							return false;
					}
				...
				case self::BZIP:
					if($files === null || $files === $this->files)
					{
						$dir = rtrim($outputFolder , '/') . '/';
						if(!is_dir($dir) && !mkdir($dir)) return false;
						if(file_put_contents($dir . basename($this->bzipFilename , '.bz2') , bzdecompress(file_get_contents($this->bzipFilename))) !== false) return 1;
						else
							return false;
					}
				...
				case self::LZMA:
					if($files === null || $files === $this->files)
					{
						...
						xzclose($fp);
						if(file_put_contents($dir . basename($this->lzmaFilename , '.xz') , $content) !== false) return 1;
						else
							return false;
					}
		...
		{
			if($expandFilesList && $fileOrFiles !== null) $fileOrFiles = self::expandFileList($this->files , is_string($fileOrFiles) ? [$fileOrFiles] : $fileOrFiles);
			...
			switch ($this->type)
			{
				case self::ZIP:
					...
					foreach ($files as $file)
					{
						...
						if($this->zip->deleteIndex($index)) $count++;
					}
					break;
			...
			switch ($this->type)
			{
				case self::ZIP:
					foreach ($files_list as $localname => $filename)
					{
						if(is_null($filename))
						{
							if($this->zip->addEmptyDir($localname) === false) return false;
						}
						else
						{
							if($this->zip->addFile($filename , $localname) === false) return false;
						}
					}
		...
		{
			if(file_exists($archiveName)) throw new Exception('Archive ' . $archiveName . ' already exists!');
			...
			if(in_array($atype , [
				...
				TarArchive::TAR_LZMA ,
				TarArchive::TAR_LZW,
			] , true)) return TarArchive::archiveFiles($fileOrFiles , $archiveName , $emulate);
			...
			switch ($atype)
			{
				case self::ZIP:
					...
					if($result !== true) throw new Exception('ZipArchive error: ' . $result);
					foreach ($files_list as $localname => $filename)
					{
						if($filename === null)
						{
				...
				case self::GZIP:
					...
					if(file_put_contents($archiveName , gzencode(file_get_contents($filename))) !== false) return 1;
				...
				case self::BZIP:
					...
					$filename = array_shift($files_list);
					if(is_null($filename)) return false; // invalid list
					if(file_put_contents($archiveName , bzcompress(file_get_contents($filename))) !== false) return 1;
