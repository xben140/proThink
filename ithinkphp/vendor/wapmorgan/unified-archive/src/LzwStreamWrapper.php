<?php
	...
	class LzwStreamWrapper
	{
		...
		{
			if(!self::$registered) stream_wrapper_register('compress.lzw' , __CLASS__);
		...
		public function stream_open($path , $mode , $options)
		{
			...
			if(self::$installed === 0)
			{
				...
				if(empty($output)) throw new \Exception(__FILE__ . ', line ' . __LINE__ . ': compress command is required');
				$this->exec('command -v uncompress' , $output);
				if(empty($output)) throw new \Exception(__FILE__ . ', line ' . __LINE__ . ': uncompress command is required');
				self::$installed = true;
			}

			$schema = 'compress.lzw://';
			if(strncasecmp($schema , $path , strlen($schema)) == 0) $path = substr($path , strlen($schema));
			...
			if(file_exists($path))
			{
				...
				if($expected_data_size <= (self::$TMP_FILE_THRESHOLD * $available_memory) && !self::$forceTmpFile && $expected_data_size < (self::$highLimit * 1024 * 1024))
				{
					$this->read();
				}
				else
				{
					...
					if(($tmp = tempnam(sys_get_temp_dir() , $prefix)) === false) throw new \Exception(__CLASS__ . ', line ' . __LINE__ . ': Could not create temporary file in ' . sys_get_temp_dir());
					if(($tmp2 = tempnam(sys_get_temp_dir() , $prefix)) === false) throw new \Exception(__CLASS__ . ', line ' . __LINE__ . ': Could not create temporary file in ' . sys_get_temp_dir());
					$this->tmp = $tmp;
					$this->tmp2 = $tmp2;
					$this->read();
				}
		...
		public function stream_close()
		{
			...
			if($this->writtenBytes > 0)
			{
				...
				{
					// compress in tmp2
					$this->exec('compress -c ' . escapeshellarg($this->tmp) . ' > ' . escapeshellarg($this->tmp2) , $output , $code);
				...
				{ // stored in local var
					...
					$process = proc_open('compress > ' . escapeshellarg($this->path) , array(
							0 => array(
								'pipe' ,
								'r',
							),
