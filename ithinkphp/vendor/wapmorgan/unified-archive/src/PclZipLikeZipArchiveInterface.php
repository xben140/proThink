...
//
	...
	class PclZipLikeZipArchiveInterface implements PclZipOriginalInterface
	{
		...
		public function create($content)
		{
			...
			{
				$options = array_combine(array_filter($options , function($v) { return (bool)$v & 2; }) , array_filter($options , function($v) { return (bool)($v - 1) & 2; }));
			}

			// filters initiation
			$filters = array();
			if(isset($options[PCLZIP_OPT_REMOVE_PATH]) && !isset($options[PCLZIP_OPT_REMOVE_ALL_PATH])) $filters[] = function(&$key , &$value) use ($options) {
				$key = str_replace($key , null , $key);
			};
			if(isset($options[PCLZIP_OPT_REMOVE_ALL_PATH])) $filters[] = function(&$key , &$value) { $key = basename($key); };
			if(isset($options[PCLZIP_OPT_ADD_PATH])) $filters[] = function(&$key , &$value) use ($options) {
				$key = rtrim($options[PCLZIP_OPT_ADD_PATH] , '/') . '/' . ltrim($key , '/');
			};
		...
		private function addSnippet($file_to_add , array $filters , $preAddCallback , $postAddCallback)
		{
			if(is_file($file_to_add) || is_dir($file_to_add))
			{
				...
				if(call_user_func($preAddCallback , $file_header) == 1)
				{
					...
					if(is_file($filename)) $this->archive->addFile($file_header->filename , $file_header->stored_filename);
					else if(is_dir($filename)) $this->archive->addEmptyDir($file_header->stored_filename);
				}
				else
				{
					//
		...
		public function extract()
		{
			...
			// <ereg> rule
			else if(isset($options[PCLZIP_OPT_BY_EREG]) && function_exists('ereg')) $selectFilter = function($key , $value) use ($options) {
