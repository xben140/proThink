<?php
	...
	class CamApplication
	{
		...
		{
			if(!UnifiedArchive::canOpenArchive($file)) throw new Exception('Could not open archive ' . $file . '. Try installing suggested packages or run `cam -f` to see formats support.');
		...
		public function checkFormats()
		{
			$types = [
				...
				'.7z'  => [
					class_exists('\Archive7z\Archive7z') ,
					'install "gemorroj/archive7z" package',
				] ,
				'.tar' => [
					class_exists('\Archive_Tar') || class_exists('\PharData') ,
					'install "phar" extension or "pear/archive_tar" package',
				] ,
				'.iso' => [
					class_exists('\CISOFile') ,
					'install "phpclasses/php-iso-file" package',
				] ,
			];
		...
		public function table($args)
		{
			...
			foreach ($archive->getFileNames() as $file)
			{
				...
				echo sprintf('%-51s | %1.1f%s | %18s' . PHP_EOL , $file_name , $size[0] , $size[1] , $this->formatDate($info->modificationTime));
			}
		...
		{
			if(strtotime('today') < $unixtime) return 'Today, ' . date('G:m' , $unixtime);
			else if(strtotime('yesterday') < $unixtime) return 'Yesterday, ' . date('G:m' , $unixtime);
			else
			{
				$datetime = new \DateTime();
				$datetime->setTimestamp($unixtime);
				if($datetime->format('Y') == date('Y')) return $datetime->format('d M, G:m');
				else
		...
		public function delete($args)
		{
			...
			foreach ($args['FILES_IN_ARCHIVE'] as $file)
			{
				...
				if($archive->deleteFiles($file) === false) echo 'Error file ' . $file . PHP_EOL;
			}
		}
		...
		public function add($args)
		{
			...
			if($added_files === false) echo 'Error' . PHP_EOL;
			else
		...
		public function create($args)
		{
			...
			{
				if(is_dir($args['ARCHIVE'])) echo $args['ARCHIVE'] . ' is a directory!' . PHP_EOL;
				else
				{
			...
			else
			{
				...
				if($archived_files === false) echo 'Error' . PHP_EOL;
				else
