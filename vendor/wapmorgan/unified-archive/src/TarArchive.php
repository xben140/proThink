<?php
	...
	class TarArchive extends BasicArchive
	{
		...
		public function __construct($fileName , $type = null)
		{
			...
			if(!self::$enabledPharData && !self::$enabledPearTar) throw new Exception('Archive_Tar nor PharData not available');

			if($type === null) $type = strtolower(pathinfo($fileName , PATHINFO_EXTENSION));

			switch ($type)
			{
				case 'gz':
				case 'tgz':
					if(self::$enabledPharData) $this->tar = new PharData($fileName);
					else
				...
				case 'tbz2':
					...
					break;
				case 'xz':
					if(!self::$enabledPharData) throw new Exception('Archive_Tar not available');
				...
				case 'z':
					...
					break;
				default:
					if(self::$enabledPharData) $this->tar = new PharData($fileName , 0 , null , Phar::TAR);
					else
		...
		public function getFileData($fileName)
		{
			...
			if($this->tar instanceof Archive_Tar)
			{
				...
				return new ArchiveEntry($fileName , $data['size'] / $this->tarCompressionRatio , $data['size'] , $data['mtime'] , in_array(strtolower(pathinfo($this->tar->path , PATHINFO_EXTENSION)) , array(
					'gz' ,
					'bz2' ,
					'xz' ,
					'Z',
			...
			else
			{
				...
				return new ArchiveEntry($fileName , $entry_info->getSize() , filesize($entry_info->getPathname()) , 0 , $entry_info->isCompressed());
			}
		...
		{
			if(!in_array($fileName , $this->files , true)) return false;
			if($this->tar instanceof Archive_Tar) return $this->tar->extractInString($fileName);
			else
		...
		{
			if($expandFilesList && $files !== null) $files = self::expandFileList($this->files , $files);
		...
		{
			if($this->tar instanceof Archive_Tar) return false;

			if($expandFilesList && $fileOrFiles !== null) $fileOrFiles = self::expandFileList($this->files , $fileOrFiles);
			...
			{
				if(!in_array($file , $this->files , true)) continue;

				if($this->tar->delete($file)) $deleted++;
			}
		...
		public function addFiles($fileOrFiles)
		{
			...
			if($this->tar instanceof Archive_Tar)
			{
				foreach ($fileOrFiles as $localname => $filename)
				{
					...
					if(is_null($filename))
					{
						if($this->tar->addString($localname , "") === false) return false;
					}
					else
					{
						if($this->tar->addModify($filename , $add_dir , $remove_dir) === false) return false;
					}
				}
			}
			else
			{
				foreach ($fileOrFiles as $localname => $filename)
				{
					if(is_null($filename))
					{
						if($this->tar->addEmptyDir($localname) === false) return false;
					}
					else
					{
						if($this->tar->addFile($filename , $localname) === false) return false;
					}
				}
			}
		...
		public static function archiveFiles($fileOrFiles , $archiveName , $emulate = false)
		{
			...
			if(self::$enabledPearTar)
			{
				...
				if(isset($tar_aname)) $tar = new Archive_Tar($tar_aname , $compression);
				else
			...
			else if(self::$enabledPharData)
			{
				...
				foreach ($fileOrFiles as $localname => $filename)
				{
					...
					{
						if($tar->addFile($filename , $localname) === false) return false;
					}
				}
		...
		protected function scanArray()
		{
			...
			if($this->tar instanceof Archive_Tar)
			{
				...
				$this->compressionRatio = $this->uncompressedFilesSize != 0 ? ceil($this->archiveSize / $this->uncompressedFilesSize) : 1;
			}
			else
			{
		...
		public function getFileResource($fileName)
		{
			...
			if($this->tar instanceof Archive_Tar) fwrite($resource , $this->tar->extractInString($fileName));
			else
