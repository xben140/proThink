	...
	abstract class BasicArchive implements AbstractArchive
	{
		...
		protected static function expandFileList($archiveFiles , $files)
		{
			...
			foreach ($files as $file)
			{
				...
				{
					if(fnmatch($file . '*' , $archiveFile)) $newFiles[] = $archiveFile;
				}
			}
		...
		protected static function createFilesList($nodes)
		{
			...
			if(is_array($nodes))
			{
				...
				foreach ($nodes as $source => $destination)
				{
					...
					// if is directory
					if(is_dir($source)) self::importFilesFromDir(rtrim($source , '/\\*') . '/*' , $destination . '/' , true , $files);
					else if(is_file($source)) $files[$destination] = $source;
				}
			...
			{
				// if is directory
				if(is_dir($nodes)) self::importFilesFromDir(rtrim($nodes , '/\\*') . '/*' , '/' , true , $files);
				else if(is_file($nodes)) $files[basename($nodes)] = $nodes;
			}
		...
		protected static function importFilesFromDir($source , $destination , $recursive , &$map)
		{
			...
			if($destination != '') $map[$destination] = null;
			foreach (glob($source , GLOB_MARK) as $node)
			{
				if(in_array(substr($node , -1) , [
						'/' ,
						'\\',
					] , true) && $recursive)
				{
		...
		{
			if(!is_file($file)) throw new \InvalidArgumentException($file . ' is not a valid file to archive');
		...
		{
			if(!is_dir($directory) || !is_readable($directory)) throw new \InvalidArgumentException($directory . ' is not a valid directory to archive');
		...
		{
			if(!is_file($file)) throw new \InvalidArgumentException($file . ' is not a valid file to add in archive');

			return ($inArchiveName !== null ? $this->addFiles([$file => $inArchiveName]) : $this->addFiles([$file])) === 1;
		}
		...
		{
			if(!is_dir($directory) || !is_readable($directory)) throw new \InvalidArgumentException($directory . ' is not a valid directory to add in archive');

			return ($inArchivePath !== null ? $this->addFiles([$directory => $inArchivePath]) : $this->addFiles([$inArchivePath])) > 0;
		}
	}
