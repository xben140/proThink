<?php
	...
	class UnifiedArchiveTest extends PhpUnitTestCase
	{
		...
		{
			if(!UnifiedArchive::canOpenType($archiveType)) $this->markTestSkipped($archiveType . ' is not supported with current system configuration');

			$test_archive_filename = __DIR__ . '/' . $archiveFileName;
			if(file_exists($test_archive_filename)) $this->assertTrue(unlink($test_archive_filename));
		...
		{
			$class = (strpos($filename , '.tar') !== false) ? 'wapmorgan\UnifiedArchive\TarArchive' : 'wapmorgan\UnifiedArchive\UnifiedArchive';

			$full_filename = self::getArchivePath($filename);

			if(!UnifiedArchive::canOpenArchive($full_filename)) $this->markTestSkipped(UnifiedArchive::detectArchiveType($full_filename) . ' is not supported with current system configuration');

			$this->assertInstanceOf($class , UnifiedArchive::open($full_filename) , 'UnifiedArchive::open() on ' . $full_filename . ' should return an object');
		}
...
//    }
		...
		protected function flattenFilesList($prefix , array $list , array &$output)
		{
			...
			{
				if(is_array($value)) $this->flattenFilesList($prefix . $name . '/' , $value , $output);
				else
